...
void map_test() {
  ...
  mystl::Vector<PAIR> v;
  for (int i = 0; i < 5; ++i) v.push_back(PAIR(i, i));
  ...
  std::cout << " m1.equal_range(2) : from <" << first.first << ", " << first.second << "> to <"
...
#else
  MAP_EMPLACE_TEST(map, Map, SCALE_M(LEN1), SCALE_M(LEN2), SCALE_M(LEN3));
#endif
  std::cout << std::endl;
  std::cout << "|---------------------|-------------|-------------|-------------|" << std::endl;
  PASSED;
...
#else
  MAP_EMPLACE_TEST(multimap, MultiMap, SCALE_S(LEN1), SCALE_S(LEN2), SCALE_S(LEN3));
#endif
  std::cout << std::endl;
  std::cout << "|---------------------|-------------|-------------|-------------|" << std::endl;
  PASSED;
